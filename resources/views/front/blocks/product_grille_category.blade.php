@foreach($data as $row)
	<div class="col-md-3 col-xs-6 product-grille">
		<div class="image">
			<a href=""><img src="{{ asset($row->cheminMedia) }}" alt="{{$row->libelleMedia}}" title="{{$row->libelleMedia}}"></a>
		</div>
		<div class="description">
			<h3 class="title">{{$row->nom}}</h3>
			<div class="libelle">{{$row->descriptionProd}}</div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-3 col-md-offset-1 col-sm-3 col-sm-offset-1 col-xs-3 col-xs-offset-1"><img src="{{ asset($row->cheminMedia) }}" class="img-responsive" alt="{{$row->libelleMedia}}" title="{{$row->libelleMedia}}"></div>
